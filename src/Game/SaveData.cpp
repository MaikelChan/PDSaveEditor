...
void PakFile::Load(uint8_t* fileBuffer, const bool isBigEndian)
{
	...
	SaveData::CalculateChecksum(&fileBuffer[8], &fileBuffer[PACK_HEADER_SIZE], headerChecksum);

	uint16_t bodyChecksum[2];
	SaveData::CalculateChecksum(&fileBuffer[PACK_HEADER_SIZE], &fileBuffer[PACK_HEADER_SIZE + pakFileHeader.bodylen], bodyChecksum);
...
void SaveData::Load(uint8_t* fileBuffer, const bool isBigEndian)
{
...
void SaveData::Save(uint8_t* fileBuffer, const bool isBigEndian)
{
	int32_t p = 0;
	...
	if (p != SAVE_DATA_SIZE - PACK_TERMINATOR_ACTUAL_SIZE - PACK_HEADER_SIZE)
...
uint8_t SaveData::GetGameFileCount() const
{
	uint8_t count = 0;
...
uint8_t SaveData::GetMultiplayerProfileCount() const
{
	uint8_t count = 0;
	for (uint8_t f = 0; f < ACTUAL_NUM_FILE_SLOTS; f++)
	{
...
uint8_t SaveData::GetMultiplayerSetupCount() const
{
	uint8_t count = 0;
	for (uint8_t f = 0; f < ACTUAL_NUM_FILE_SLOTS; f++)
	{
...
GameFile* SaveData::GetFirstUnusedGameFile(uint8_t* outFileIndex)
{
	for (uint8_t f = 0; f < ACTUAL_NUM_FILE_SLOTS; f++)
	{
...
MultiplayerProfile* SaveData::GetFirstUnusedMultiplayerProfile(uint8_t* outFileIndex)
{
	for (uint8_t f = 0; f < ACTUAL_NUM_FILE_SLOTS; f++)
	{
...
MultiplayerSetup* SaveData::GetFirstUnusedMultiplayerSetup(uint8_t* outFileIndex)
{
	for (uint8_t f = 0; f < ACTUAL_NUM_FILE_SLOTS; f++)
	{
...
void SaveData::CalculateChecksum(uint8_t* start, uint8_t* end, uint16_t* checksum)
{
	uint8_t* ptr;
	uint32_t salt = 0;
...
void SaveData::PrintFileInfo(uint8_t* fileBuffer, const bool isBigEndian) const
{
